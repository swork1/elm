...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{aB: false, aD: r.aD, aH: r.aH, _: r._, aM: r.aM, aW: r.aW, av: r.av, m: r.m}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aD: $elm$http$Http$emptyBody, aH: r.aH, _: _List_Nil, aM: 'GET', aW: $elm$core$Maybe$Nothing, av: $elm$core$Maybe$Nothing, m: r.m});
};
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$PhotoGroove$Photo = F3(
	function (url, size, title) {
		return {ar: size, au: title, m: url};
	});
...
var $author$project$PhotoGroove$initialCmd = $elm$http$Http$get(
	{
		...
		m: 'http://elm-in-action.com/photos/list.json'
	});
var $author$project$PhotoGroove$Loading = {$: 0};
var $author$project$PhotoGroove$Medium = 1;
var $author$project$PhotoGroove$initialModel = {A: '', C: 1, D: 5, F: 5, G: 5, i: $author$project$PhotoGroove$Loading};
	...
	switch (_v0.$) {
		case 1:
			...
			var filters = _List_fromArray(
				[
					{L: model.D / 11, M: 'Hue'},
					{L: model.G / 11, M: 'Ripple'},
					{L: model.F / 11, M: 'Noise'}
				]);
			return _Utils_Tuple2(
				model,
					...
					if (photos.b) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									i: $author$project$PhotoGroove$Errored('No Photos')
								}),
							$elm$core$Platform$Cmd$none);
					}
				} else {
...
var $author$project$PhotoGroove$ClickedSurpriseMe = {$: 4};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					thumb.au + (' [' + ($elm$core$String$fromInt(thumb.ar) + ' KB]'))),
	...
	function (photos, selectedUrl, model) {
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidHue, 'Hue', model.D),
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidRipple, 'Ripple', model.G),
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidNoise, 'Noise', model.F)
					])),
				A2(
				$elm$html$Html$h3,
				_List_Nil,
				...
				A2(
					$elm$core$List$map,
					function (size) {
						return A2($author$project$PhotoGroove$viewSizeChooser, size, model.C);
					},
					_List_fromArray(
						[0, 1, 2]))),
				A2(
				...
				_List_fromArray(
					[
						...
						$author$project$PhotoGroove$sizeToString(model.C))
					]),
				A2(
					$elm$core$List$map,
